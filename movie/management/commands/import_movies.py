...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f"Error decoding JSON in file {file_path}")
                )
            return
        ...
        for item in data:
            try:
                ...
                # Handle actors
                actor_names = [
                    name.strip() for name in item.get("Actors", "").split(",")
                    ]
                ...
                # Handle genres
                genre_names = [
                    name.strip() for name in item.get("Genre", "").split(",")
                    ]
        ...
        if valid_objects:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully imported {len(valid_objects)} records"
                    )
                )
        else:
        ...
        if invalid_count > 0:
            self.stdout.write(
                self.style.WARNING(f"Skipped {invalid_count} invalid records")
                )
